...
def localization(ser, NUM_PARTICLES=2000):
    ...
    # There are some big mistakes in naming the respective angles as directions hre, for example tje 180 degrees os defined as sensor_fronl...
    # if 0deg is sensor_front, then 30deg is NOT sensor right, it is sensor_frontr, 90 degrees is NOT sensor_left should be sensor_right
    # 150 deg is not sensor_back should be sensor_backr, 180 deg should not be sensor_frontl, should be sensor_back, 210 deg is NOT sensor_frontr, should be sensor_backl
    # 270 deg is NOT sensor_backl it is sensor_left, 330deg is NOT sensor_backr, but it is sensor frontl
    # explains why the rover sees the front as threadable but actually is facing a wall
    ...
    try:
        for i in range(NUM_STEPS):
            ...
            if sensor_front > threshold and sensor_frontr > diag_threshold and sensor_frontl > diag_threshold:
                ...
                ("rotate_left small and move forward")
                # Send a drive forward command with left correction
                transmit(packetize(CMD_LIST[1]))
                time.sleep(LOOP_PAUSE_TIME)
                transmit(packetize(CMD_LIST[0]))
