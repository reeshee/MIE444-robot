...
def initialize_particles(num_particles):
    ...
    # (x_min, x_max, y_min, y_max) for each wall segment in the maze
    wall_ranges = [
        (8, 26, 20, 40),    # (12, 24, 24, 36)
        (20, 40, 8, 26),    # (24, 36, 12, 24)
        (34, 64, 20, 40),   # (36, 60, 24, 36)
        (44, 62, 0, 16),    # (48, 60, 0, 12)
        (70, 86, 0, 16),    # (72, 84, 0, 12)
        (68, 86, 20, 48),   # (72, 84, 24, 48)
        (0, 96, 0, 4),      # Top Wall
        (94, 96, 0, 48),   # Right Wall
        (0, 96, 44, 48),    # Bottom Wall
        (0, 4, 0, 48)       # Left Wall
    ]
...
def estimate_robot_position(particles, weight_threshold=0):

    weight_threshold = 1.0/len(particles)
    
    if len(particles) == 50:
        weight_threshold = 0
        high_weight_particles = [p for p in particles if p.weight > weight_threshold]
    else:
        # Identify high-weight particles excluding those erroneously set to 1.0
        high_weight_particles = [p for p in particles if p.weight > weight_threshold]
        temp_weight = [p.weight for p in high_weight_particles]
        print("HIGH WEIGHT PARTICLES WEIGHTS: ", temp_weight)
