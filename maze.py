...
class Maze:
    ...
    def get_distance_to_wall(self, x, y, direction):
        """Calculates the shortest distance from (x, y) to any wall in the given direction."""
        particle_point = Point(x, y)
        min_distance = float('inf')

        # Create a direction vector
        dx = np.cos(np.radians(direction))
        dy = np.sin(np.radians(direction))
        
        for wall in self.reduced_walls:
            # Convert wall segment into a line
            line = LineString(wall)

            # Extend the direction line from particle and find the intersection
            extended_line = LineString([particle_point, (x + dx * 1000, y + dy * 1000)])
            if line.intersects(extended_line):
                intersection_point = line.intersection(extended_line)
                distance = particle_point.distance(intersection_point)
