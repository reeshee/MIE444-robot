...
# Function to draw the maze (similar to your other scripts)
def draw_maze():
    maze.draw_walls(canvas)  # Implement this method in your Maze class

# Function to draw particles
def draw_particles_on_canvas():
...
try:
    for i in range(NUM_STEPS):
        ...
        elif sensor_front < threshold:
            # Obstacle detected ahead, needs avoidance
            if sensor_left > sensor_right:
                # Turn left if there is more space on the left
                transmit(packetize(CMD_LIST[4]))  # Turn left -18 degrees
            else:
                # Turn right if there is more space on the right
                transmit(packetize(CMD_LIST[5]))  # Turn right +18 degrees
            time.sleep(LOOP_PAUSE_TIME)
            [responses, time_rx] = receive()
            move_particles(particles, move_distance=0, rotation_change=-18 if sensor_left > sensor_right else 18)

        elif sensor_left < diag_threshold or sensor_right < diag_threshold:
            # Side obstacle detected, make a gentle turn to avoid
            if sensor_left < diag_threshold and sensor_right >= diag_threshold:
                # Turn slightly right to avoid left obstacle
                transmit(packetize(CMD_LIST[2]))  # Turn right +10 degrees
            elif sensor_right < diag_threshold and sensor_left >= diag_threshold:
                # Turn slightly left to avoid right obstacle
                transmit(packetize(CMD_LIST[1]))  # Turn left -10 degrees
            else:
                # If both sides are close, reverse a bit first
                transmit(packetize(CMD_LIST[3]))  # Reverse
            time.sleep(LOOP_PAUSE_TIME)
            [responses, time_rx] = receive()
            move_particles(particles, move_distance=-0.5 if sensor_left < diag_threshold and sensor_right < diag_threshold else 0,
                           rotation_change=-10 if sensor_right < diag_threshold else 10)

        else:
            # Default forward movement if nothing is too close
            transmit(packetize(CMD_LIST[0]))
            time.sleep(LOOP_PAUSE_TIME)
            [responses, time_rx] = receive()
            move_particles(particles, move_distance=1.2, rotation_change=0)

        # If collision is imminent from multiple directions (front, left, right), back up
        if sensor_front < threshold or sensor_frontl < diag_threshold or sensor_frontr < diag_threshold:
            if sensor_front < threshold:
                # If there's an obstacle straight ahead
                transmit(packetize(CMD_LIST[3]))  # Reverse
                time.sleep(LOOP_PAUSE_TIME)
                [responses, time_rx] = receive()
                move_particles(particles, move_distance=-0.5, rotation_change=0)
            elif sensor_left > sensor_right:
                # Turn left if more space is on the left
                transmit(packetize(CMD_LIST[4]))  # Turn left -18 degrees
            elif sensor_right > sensor_left:
                # Turn right if more space is on the right
                transmit(packetize(CMD_LIST[5]))  # Turn right +18 degrees
            else:
                # If space is even on both sides, just reverse
                transmit(packetize(CMD_LIST[3]))  # Reverse
            time.sleep(LOOP_PAUSE_TIME)
            [responses, time_rx] = receive()
            move_particles(particles, move_distance=0, rotation_change=-18 if sensor_left > sensor_right else 18)
        ...
        update_particle_weights(particles, robot_readings)  # Calculate particle weights
    
        ess = calculate_ess(particles)
        if ess > HIGH_ESS * NUM_PARTICLES:
            update_particle_weights(particles, robot_readings, sigma=6)  # Calculate particle weights
            print("YESSSIIIIRRRRRR")
            particles = resample_particles(particles)   # Regenerate particles with weight
            convergence_condition += 1
            RESAMPLE_INTERVAL = 1
        elif iteration % RESAMPLE_INTERVAL == 0 and iteration > 0:
    ...
    pygame.quit()
    print("Pygame has been quit gracefully.")
